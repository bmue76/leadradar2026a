...
## TP 7.8 — Device Provisioning + Device Licenses (GoLive MVP)

### MobileProvisionToken (12h)
- Token TTL: **12 Stunden** (`expiresAt`)
- Status: `ACTIVE | REVOKED | USED`
- Token ist device-gebunden (`deviceId`) und kann bei ACTIVE mehrfach versendet werden.
- Redeem setzt `USED`, `usedAt`, `usedByDeviceId`, und löscht `tokenPlaintext`.

### DeviceLicense (History = Source of Truth)
- Pro Kauf/Aktivierung entsteht ein **neuer** DeviceLicense Datensatz (History).
- Verlängerung: `base = max(now, currentEndsAt)` → `endsAt = base + 30/365 Tage`.
- Stripe Idempotenz über `stripeCheckoutSessionId` (unique).
- Pending-Model (MVP): `note = "stripe_pending"` bedeutet **gekauft, startet erst bei App-Aktivierung**.
