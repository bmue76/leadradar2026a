...
type BrandingState =
  | { kind: "loading" }
  | { kind: "ready"; tenantName: string | null; tenantSlug: string | null; hasLogo: boolean; logoDataUrl: string | null }
...
export function useBranding() {
  ...
  const refresh = useCallback(async () => {
    try {
      ...
      if (res.ok !== true) {
        const msg = typeof (res as Record<string, unknown>).message === "string" ? String((res as any).message) : "Request failed";
